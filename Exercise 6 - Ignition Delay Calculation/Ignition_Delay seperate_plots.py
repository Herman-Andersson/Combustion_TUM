...
for plot_idx in range(3):
    ...
    data_to_plot(exp_datasets[plot_idx], markers, colors, ax, labels_pressure)
    ...
    ax.set_xlabel(r'$\frac{1000}{T}$ [1/K]')
    ax.set_ylabel('Ignition Delay [μs]')
    ax.set_title(f'Ignition Delay Time for φ = {phi[plot_idx]}')
