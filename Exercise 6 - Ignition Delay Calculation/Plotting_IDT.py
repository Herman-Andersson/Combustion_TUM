...
# Find the closest temperature point to our target
target_T = 1000 / target_xaxis  # This gives us T ≈ 1818.18 K
closest_T_index = np.argmin(np.abs(T - target_T))
actual_T = T[closest_T_index]
actual_xaxis = 1000/actual_T

print(f"Target xaxis value: {target_xaxis}")
print(f"Target temperature: {target_T:.2f} K")
print(f"Closest available temperature: {actual_T:.2f} K")
print(f"Actual xaxis value: {actual_xaxis:.4f}")
print(f"Temperature index: {closest_T_index}")
print()

# Note: You'll need to run your main calculation first to populate store_arrays
# Then use the following code to extract the values:

print("IDT Values at xaxis ≈ 0.55:")
print("=" * 60)
print(f"{'Phi':<6} {'1 atm':<12} {'3 atm':<12} {'5 atm':<12} {'10 atm':<12}")
print("-" * 60)

# This is the extraction code you'll use after running your main calculation:
for i, phi_val in enumerate(phi):
    idt_values = []
    for j in range(len(P)):
        # Extract IDT value at the closest temperature index
        idt_val = store_arrays[i][j][closest_T_index]
        idt_values.append(idt_val)
    
    print(f"{phi_val:<6.1f} {idt_values[0]:<12.2f} {idt_values[1]:<12.2f} {idt_values[2]:<12.2f} {idt_values[3]:<12.2f}")

print()
print("Units: IDT values are in microseconds (μs)")
print("Pressures: 1 atm, 3 atm, 5 atm, 10 atm")