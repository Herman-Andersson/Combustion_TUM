...
T = np.linspace(700, 2000, 10)  # temperature in K // two present cause of troubleshooting, reamined incase of meachism errors
...
idT_rmg = np.zeros(len(T))
idT_R17 = np.zeros(len(T))
idT_R24 = np.zeros(len(T))
...
print("\nCalculating IDT for RMG 99% conv (700-2000K)...")
calculate_Ignition_Delay_simple(T, gas_rmg, "RMG 95% conv", idT_rmg, fuel_rmg)

print("\nCalculating IDT for Gri3 Reduced 17 ...")
calculate_Ignition_Delay_simple(T, gas_R17, "GRI-Mech 3.0", idT_R17, fuel_gri)

print("\nCalculating IDT for Gri3 Reduced 24 ...")
calculate_Ignition_Delay_simple(T, gas_R24, "GRI-Mech 3.0", idT_R24, fuel_gri)
# -------------------- Plot Section --------------------------------
...
ax.semilogy(xaxis, idT_gri, 'b-', linewidth=2, label='GRI-Mech 3.0', marker=None)
ax.semilogy(xaxis, idT_rmg, 'g-', linewidth=2, label='RMG 99% conv (700-2000K)', marker=None)
ax.semilogy(xaxis, idT_R17, 'r-', linewidth=2, label='GRI3 Reduced 17', marker=None)
ax.semilogy(xaxis, idT_R24, 'y-', linewidth=2, label='GRI3 Reduced 24', marker=None)
...
plt.savefig('/home/brukare/Combustion/RMG_Reduced/Analysis_scripts/ignition_delay_reduced_zoomed.png', dpi=300)
...
print(f"RMG 99% conv (700-2000K):        {gas_rmg.n_species} species, {gas_rmg.n_reactions} reactions")
print(f"GRI reduced 17:        {gas_R17.n_species} species, {gas_R17.n_reactions} reactions")
print(f"GRI reduced 24:        {gas_R24.n_species} species, {gas_R24.n_reactions} reactions")
print("="*60)