...
for i in range(len(Phi)):
    NO_conc[i] = NO_concentration(T_store_AIR[i])
    NO_frac[i] = concentration_to_molar_frac(NO_conc[i], T_store_AIR[i])
    NO_ppm[i] = convert_to_ppm(NO_frac[i], T_store_AIR[i])
...
ax1.plot(Phi, T_store_AIR, color=color, label='AFT')
ax1.axhline(y=max(T_store_AIR), color="orange", linestyle='--', label='AFT Max')
...
ax2.plot(Phi, NO_ppm, color=color, label='NO (ppm)')
