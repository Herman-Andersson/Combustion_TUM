...
# Helper function to integrate c_p from T_0 to T
def integrate_cp(cp_func, T):
    result, _ = quad(cp_func, T_0, T)
    return result

Phi = np.arange(0.1, 2.1, 0.01)
...
for index in range(len(Phi)):
    ...
    if Phi[index] <= 1:  # Fuel lean mixture
        ...
        while epsilon > 0.01:
            def equation(T):
                H_P = (b*(h_CO2 + integrate_cp(c_p_CO2, T)) + 
                       d*(h_H2O + integrate_cp(c_p_H2O, T)) + 
                       f*(h_O2 + integrate_cp(c_p_O2, T))
                        + 3.76*a*(h_N2 + integrate_cp(c_p_N2, T)))
                return H_R - H_P
        ...
        while epsilon > 0.01:
            ...
            def equation(T):
                H_P = (B*(h_CO2 + integrate_cp(c_p_CO2, T)) + 
                       C*(h_CO + integrate_cp(c_p_CO, T)) + 
                       D*(h_H2O + integrate_cp(c_p_H2O, T)) + 
                       E*(h_H2 + integrate_cp(c_p_H2, T)) 
                        + 3.76*a*(h_N2 + integrate_cp(c_p_N2, T)))
                return H_R - H_P
...
plt.title('Methane combustion', fontsize=14)
